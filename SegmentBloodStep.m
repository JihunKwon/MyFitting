function outputParams=SegmentBloodStep(inputParams)
...
qt=round(size(perfdata,1)/4);
sigbase=squeeze(mean(perfdata(1:4,incROI)));
...
[~,I]=max(peaksharpness);
arteryindex=peaksharpness(I-bloodpixnum);
bloodindex = find(bloodCharacteristics > arteryindex); 
bloodi=zeros(sizeperf(2:end));
